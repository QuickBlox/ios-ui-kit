...
class ImageCache {
    ...
    public func imageFromCache(_ key: String) -> UIImage? {
        lock.lock()
        defer { lock.unlock() }
        ...
        do {
            ...
            for file in directoryContents {
                do {
                    ...
                    debugPrint("[ImageCache] clearCache error: \(error)")
                }
            }
        } catch let error {
            debugPrint("[ImageCache] error \(error.localizedDescription)")
        }
    }
